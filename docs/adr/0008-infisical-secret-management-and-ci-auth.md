...
The platform previously centered its active secret story on `1Password` plus `fnox`. The repo now needs one current secret system that works for:
...
### Continue with `1Password` plus `fnox`

- simpler for purely local human use
- weaker fit for unattended CI and shared automation
